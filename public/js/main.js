let Anchor = {
  ...
  Anchor.linky = $('.linky').on('click', Anchor.toggleSlidey);
		...
		// Hide slidey if we clicked outside the slidey and outside the Menu button.
  $('.main-wrap').on('click', function(event) {
  if (!$(event.target).hasClass('categories-menu')) {
  Anchor.hideSlidey(event);
}
});
  ...
  self.slidey.css('z-index', me.hasClass('active') ? 1 : -1);
